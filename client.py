...
async def safe_call(label: str, session: ClientSession, tool: str, *payloads: List[Dict[str, Any]], timeout: float = 8.0):
    try:
        return await asyncio.wait_for(session.call_tool(tool, *payloads), timeout=timeout)
    ...
    async with AsyncExitStack() as stack:
        ...
        for label, path in [
            ...
            if os.path.exists(path):
                try:
                    read, write = await stack.enter_async_context(stdio_client("python", path))
                    sess = ClientSession(read, write)
                    await stack.enter_async_context(sess)
                    sessions[label] = sess
                except Exception as e:
                    print(f"[warn] {label} 서버 스폰 실패: {e}", file=sys.stderr)

        # --- 1) 이미지 입력 처리 ---
        caption_res = None
        meta_res = None
        if is_file(args.image) and sessions.get("caption") and sessions.get("exif"):
            caption_res = await safe_call("caption", sessions["caption"], "caption_image", {"path": args.image}, timeout=args.timeout)
            meta_res = await safe_call("exif", sessions["exif"], "extract_image_metadata", {"path": args.image}, {"use_open_meteo": True}, timeout=args.timeout)

        # --- 2) 플레이리스트 입력 처리 ---
        playlist_res_for_synth = None
        if args.playlist:
            if is_spotify_playlist(args.playlist) and sessions.get("playlist"):
                playlist_stats = await safe_call("playlist", sessions["playlist"], "analyze_playlist", {"spotify_url": args.playlist}, timeout=args.timeout)
                playlist_res_for_synth = playlist_stats or None
            elif sessions.get("trackinfo"):
                if is_file(args.playlist):
                    tp = await safe_call("trackinfo", sessions["trackinfo"], "resolve_text_playlist", {"path": args.playlist}, timeout=args.timeout)
                else:
                    lines = [s.strip() for s in args.playlist.replace(";", "").replace(",", "").splitlines() if s.strip()]
                    tp = await safe_call("trackinfo", sessions["trackinfo"], "resolve_text_playlist", {"lines": lines}, timeout=args.timeout)
                playlist_res_for_synth = coerce_text_playlist_to_stats(tp) if tp else None

        # --- 3) MBTI 입력 처리 ---
        mbti_res = None
        if args.mbti and sessions.get("mbti"):
            mbti_res = await safe_call("mbti", sessions["mbti"], "infer_mbti_traits", {"mbti": args.mbti.upper()}, timeout=args.timeout)
        ...
        if args.diary and sessions.get("diary"):
            if is_file(args.diary):
                ...
                text = args.diary
            diary_res = await safe_call("diary", sessions["diary"], "summarize_diary", {"text": text, "language": "ko"}, timeout=args.timeout)
        ...
        if sessions.get("synth"):
            final = await safe_call("synth", sessions["synth"], "synthesize_dayline", payload, timeout=max(6.0, args.timeout))
        ...
        print(json.dumps({
            "dayline": final["line"],
            "used": {
                "caption": bool(caption_res and caption_res.get("caption")),
